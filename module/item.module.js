...
class Item {
    async create(prop){
        const {ean, brand, name, type, under_type, size, material, price, cent, gender, color, fasion, cutting, img} = prop
        ...
        [ean, brand.toLowerCase(), name.toLowerCase(), type.toLowerCase(), under_type.toLowerCase(),
        size, material.toLowerCase(), price, cent, gender.toLowerCase(), color.toLowerCase(),
        fasion.toLowerCase(), cutting.toLowerCase(), img])
    ...
    async get(result, query){
        class Params{
            ...
            async send(sqlWHERE, values) {
                if(this.results){
                    const getItem = await db.query(
                        `SELECT * FROM items ${sqlWHERE}`, values)
                    return getItem.rows
                }else{
            ...
            async type(gender, type){
                const result = await this.send('WHERE gender = $1 AND type = $2', [gender, type])
                return result
            }
            async gender(gender){
    ...
    async search(val){
        const getItem = await db.query('SELECT * FROM items WHERE name = $1 OR brand = $1', [val])
        return getItem.rows
    }
