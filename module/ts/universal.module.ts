...
class Modele{
    ...
    async add(table: string, value: [{keys: string, value: any}]){
        try{
            ...
            console.log(sql)
            await db.query(sql, value.map(e => e.value))
            return true
        }catch(e){
            console.error(e)
            return false
    ...
    async getOne(table: string, column: string, valueSearch: string){
        try{
            const getItem = await db.query(`SELECT * FROM ${table}
             WHERE ${column} = $1 `, [valueSearch])
    ...
    async search(table: string, prop: [{column: string, value: string[]}],
         sort: string | boolean, limit: string | boolean) {
        try{
            const sql = `SELECT * FROM ${table} ${prop.map((e,i) => i === 0 ? 
    ...
    async update(table: string, columnSerach: string, valueSearch: string,
         columnSET: string, valueSet: string, returning: boolean){
        try{
            ...
            if(returning) return data.rows[0]; else return true
        }catch(e){
            console.error(e)
            return false
        }
